...
def wasserstein_distance(box1, box2, xywh=True, eps=1e-7):
    ...
    NWD models bounding boxes as 2D Gaussian distributions and computes the
    Wasserstein distance between them, which is particularly effective for
    small object detection as it provides more stable gradients.

    Reference: "NWD: A Novel Normalized Wasserstein Distance for Small Object Detection"

    Args:
        box1 (torch.Tensor): Predicted bounding boxes, shape (..., 4).
        box2 (torch.Tensor): Ground truth bounding boxes, shape (..., 4).
        xywh (bool, optional): If True, boxes are in (x, y, w, h) format.
                               If False, boxes are in (x1, y1, x2, y2) format. Defaults to True.
        eps (float, optional): Small value to avoid division by zero. Defaults to 1e-7.

    Returns:
        (torch.Tensor): Normalized Wasserstein Distance values.

    Mathematical formulation:
        For two 2D Gaussian distributions N(μ₁, Σ₁) and N(μ₂, Σ₂):
        W²(N₁, N₂) = ||μ₁ - μ₂||² + ||Σ₁^(1/2) - Σ₂^(1/2)||²_F

        Where:
        - μ = (cx, cy): center coordinates
        - Σ = diag(w²/12, h²/12): covariance matrix (assuming uniform distribution)
        - ||·||²_F: Frobenius norm squared

        NWD = exp(-√(W²) / C)
        where C is a normalization constant (typically set to the image diagonal)
    """
    # Convert boxes to center format if needed
    if xywh:
        # box format: (x_center, y_center, width, height)
        (x1, y1, w1, h1), (x2, y2, w2, h2) = box1.chunk(4, -1), box2.chunk(4, -1)
    else:
        # box format: (x1, y1, x2, y2) -> convert to (cx, cy, w, h)
        b1_x1, b1_y1, b1_x2, b1_y2 = box1.chunk(4, -1)
        b2_x1, b2_y1, b2_x2, b2_y2 = box2.chunk(4, -1)
    ...
    # Center distance: ||μ₁ - μ₂||²
    center_dist = (x1 - x2).pow(2) + (y1 - y2).pow(2)

    # Covariance distance: ||Σ₁^(1/2) - Σ₂^(1/2)||²_F
    # For diagonal covariance matrices with uniform distribution:
    # Σ = diag(w²/12, h²/12)
    # Σ^(1/2) = diag(w/√12, h/√12)
    # ||Σ₁^(1/2) - Σ₂^(1/2)||²_F = (w1/√12 - w2/√12)² + (h1/√12 - h2/√12)²
    #                              = (w1 - w2)²/12 + (h1 - h2)²/12

    wh_dist = ((w1 - w2).pow(2) + (h1 - h2).pow(2)) / 12.0

    # Wasserstein distance squared
    wasserstein_2 = center_dist + wh_dist

    # Normalized Wasserstein Distance
    # Use a constant C for normalization (set to 1.0 for simplicity, can be tuned)
    # NWD = exp(-√(W²) / C)
    # For loss computation, we use: 1 - NWD = 1 - exp(-√(W²) / C)

    # To avoid numerical issues, we clamp the distance
    wasserstein_2 = wasserstein_2.clamp(min=eps)

    # Return the Wasserstein distance (not normalized yet, will be done in loss function)
    return wasserstein_2.sqrt()
