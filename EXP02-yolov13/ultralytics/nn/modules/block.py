...
class ECA(nn.Module):
    """
    Optimized Efficient Channel Attention (ECA) module for small object detection.

    ECA是一种轻量级的通道注意力机制，通过1D卷积自适应地学习通道间的依赖关系。
    相比SE模块，ECA避免了降维操作，参数量更少，同时保持了良好的性能。

    Step 2 优化：添加双路池化（AvgPool + MaxPool）增强特征表达能力。

    论文: ECA-Net: Efficient Channel Attention for Deep Convolutional Neural Networks
    链接: https://arxiv.org/abs/1910.03151

    Attributes:
        kernel_size (int): 1D卷积的卷积核大小，通过通道数自适应计算

    Methods:
        forward: 对输入特征图应用通道注意力
    ...
    def __init__(self, channels, gamma=1.5, b=1):
        """
        初始化ECA模块

        Args:
            channels (int): 输入特征图的通道数
            gamma (float): 用于自适应计算卷积核大小的参数，优化为1.5（原2.0）
            b (int): 用于自适应计算卷积核大小的参数，默认为1
        """
        super().__init__()
        # 自适应计算卷积核大小
        # k = |log2(C) / gamma + b / gamma|_odd
        t = int(abs((math.log(channels, 2) + b) / gamma))
        k = t if t % 2 else t + 1  # 确保卷积核大小为奇数
        k = max(3, min(k, 9))  # Step 2优化：强制限制k在[3, 9]范围内
    ...
    def forward(self, x):
        ...
        Returns:
            Tensor: 应用通道注意力后的特征图，形状为 (B, C, H, W)
        """
        # Step 2优化：AvgPool + MaxPool 双路池化
        ...
        # 处理平均池化分支
        # 调整形状以适应1D卷积: (B, C, 1, 1) -> (B, 1, C)
        avg_out = avg_out.squeeze(-1).transpose(-1, -2)
        # 1D卷积学习通道间依赖: (B, 1, C) -> (B, 1, C)
...
class WeightedFeatureFusion(nn.Module):
    """
    加权特征融合模块 (Weighted Feature Fusion)

    该模块用于Neck中的高分辨率特征融合，通过可学习的权重自适应地融合不同来源的特征。
    相比简单的Concat操作，加权融合能够动态调整不同特征的贡献度，提升融合效果。

    支持两种融合模式：
    1. 'sum': 加权求和融合
    2. 'concat': 先Concat再加权（通过1x1卷积实现）

    Attributes:
        num_inputs (int): 输入特征的数量
        fusion_type (str): 融合类型，'sum' 或 'concat'
        weights (nn.Parameter): 可学习的融合权重
    ...
    Examples:
        >>> import torch
        >>> # 加权求和融合
        >>> wff_sum = WeightedFeatureFusion(num_inputs=2, fusion_type='sum')
        >>> x1 = torch.randn(2, 128, 160, 160)
        >>> x2 = torch.randn(2, 128, 160, 160)
        >>> output = wff_sum([x1, x2])
        >>> print(output.shape)
        torch.Size([2, 128, 160, 160])

        >>> # Concat融合
        >>> wff_concat = WeightedFeatureFusion(num_inputs=2, fusion_type='concat', channels=128)
        >>> output = wff_concat([x1, x2])
        >>> print(output.shape)
        torch.Size([2, 128, 160, 160])
    """

    def __init__(self, num_inputs=2, fusion_type='sum', channels=None):
        """
        初始化加权特征融合模块

        Args:
            num_inputs (int): 输入特征的数量，默认为2
            fusion_type (str): 融合类型，'sum' 或 'concat'，默认为'sum'
            channels (int): 当fusion_type='concat'时，输出通道数（必需）
        """
        super().__init__()
        ...
        # 如果是concat模式，添加1x1卷积进行通道调整
        if fusion_type == 'concat':
            assert channels is not None, "channels must be specified for concat fusion"
            self.conv = Conv(channels * num_inputs, channels, k=1, s=1)
    ...
    def forward(self, x):
        ...
        Args:
            x (list): 输入特征列表，每个元素形状为 (B, C, H, W)

        Returns:
            Tensor: 融合后的特征图
        """
        # 使用Softmax归一化权重（自动确保非负且和为1，梯度流动更好）
        weights = torch.softmax(self.weights, dim=0)

        if self.fusion_type == 'sum':
            # 检查通道数是否一致，如果不一致则需要对齐
            ...
            if len(set(channels)) > 1:
                ...
                # 延迟初始化通道对齐卷积
                if self.align_convs is None:
                    self.align_convs = nn.ModuleList([
                        Conv(c, max_channels, k=1, s=1) if c != max_channels else nn.Identity()
                        for c in channels
                    ])
                    # 将align_convs移到与输入相同的设备
                    self.align_convs = self.align_convs.to(x[0].device)

                # 对齐通道数
                x = [conv(feat) for conv, feat in zip(self.align_convs, x)]

            # 加权求和融合
            output = sum(w * feat for w, feat in zip(weights, x))
